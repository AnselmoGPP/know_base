# Mathematics

<br>![maths image](https://raw.githubusercontent.com/AnselmoGPP/know_base/master/resources/maths.jpg)

## Table of Contents
+ [Useful links](#useful-links)
+ [Algebra](#algebra)
+ [Calculus](#calculus)
+ [Discrete mathematics](#discrete-mathematics) (-> work in progress)
+ [Introduction to statistics](#introduction-to-statistics)
+ [Statistical inference](#statistical-inference)


## Useful links

- [Desmos](https://www.desmos.com/calculator?lang=en) (2D calculator)
- [Geogebra](https://www.geogebra.org/3d?lang=pt) (3D calculator)
- Gareth J. Janacek, Mark Lemmon Close (2011) _**Mathematics for computer scientists**_. Ventus publishing ApS. Retrieved from [here](https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=&cad=rja&uact=8&ved=2ahUKEwjt-IT_yJ6EAxUJSKQEHZgQD3AQFnoECCgQAQ&url=https%3A%2F%2Fdvikan.no%2Fntnu-studentserver%2Fkompendier%2Fgentle-introduction-to-mathematics-for-computer.pdf&usg=AOvVaw1bF0XvB22PQUgVGzIDwNTC&opi=89978449).


## Algebra

Resources:

- J. Abramson et al.(2021) _**Algebra and trigonometry (2e)**_. OpenStax.org. Retrieved from [here](https://openstax.org/details/books/algebra-and-trigonometry-2e)

Chapters, sections, and exercises:

- **Functions basics**
  - 3 Functions:
    - 3.1 Functions and function notation (exercises: 1, 11, 21, 32, 41)
    - 3.2 Domain and range (1, 11, 21, 31, 41)
    - 3.3 Rates of change and behaviour of graphs (1, 11, 21, 31, 41)
- **Linear & Quadratic functions**
  - 4 Linear functions:
    - 4.1 Linear functions (1, 11, 21, 31, 41, 51, 61, 71, 81, 91, 101, 111, 121)
  - 5 Polynomial and rational functions:
    - 5.1 Quadratic functions (1, 11, 21, 31, 41, 51, 61, 71)
- **Polynomial functions**
  - 5 Polynomial and rational functions:
    - 5.2 Power functions and polynomial functions (1, 11, 21, 31, 41, 51, 67, 69)
    - 5.3 Graphs of polynomial functions (1, 11, 21, 31, 41, 51, 61, 71, 75, 77)
    - 5.4 Dividing polynomials (1, 11, 21, 31, 41, 45, 51, 61, 65, 67, 71)
    - 5.5 Zeros of polynomial functions (1, 11, 21, 31, 41, 51, 57, 59, 61, 71, 77)
    - 5.6 Rational functions (1, 11, 21, 31, 41, 51, 61, 71, 81)
- **Composition & Inverses**
  - 3 Functions:
    - 3.4 Composition of functions (1, 11, 21, 31, 41, 51, 61, 71, 81, 91, 93, 97)
    - 3.7 Inverse functions (1, 11, 21, 31, 41, 43, 47)
- **Exponential & Logarithmic functions**
  - 6 Exponential and logarithmic functions:
    - 6.1 Exponential functions (3, 13, 23, 33, 63)
    - 6.2 Graphs of exponential functions (1, 11, 21, 31)
    - 6.3 Logarithmic functions (1, 11, 21, 31, 41, 51, 61, 65)
    - 6.4 Graphs of logarithmic functions (1, 11, 21, 31, 41, 51, 57)
    - 6.5 Logarithmic properties (1, 11, 21, 31, 41)
    - 6.6 Exponential and logarithmic equations (1, 11, 21, 31, 41, 51, 61, 71, 79)
    - 6.7 Exponential and logarithmic models (17, 19, 21)
- **Systems of equations**
  - 11 Systems of equations and inequalities:
    - 11.1 Systems of linear equations: Two variables (1, 11, 21, 31, 41, 47, 55, 59, 65, 69)
    - 11.2 Systems of linear equations: Three variables (1, 11, 21, 31, 41, 47, 55, 59, 65, 69)
    - 11.3 Systems of nonlinear equations and inequalities: Two variables (1, 7, 11, 21, 31, 41, 47, 53, 57)
    - 11.6 Solving systems with Gaussian elimination (1, 7. 11, 21, 31, 41, 47, 55, 59)
- **Foundations of trigonometry**
  - 7 The unit circle: Sine and Cosine functions:
    - 7.1 (1, 11, 25, 31, 35, 43, 47, 51, 55, 65, 71)
    - 7.2 (3, 11, 21, 31, 41, 51, 55)
    - 7.3 (1, 7, 11, 23, 35, 43, 51, 61, 71, 81, 91, 101)
  - 8 Periodic functions:
    - 8.1 (1, 11, 21, 31, 43)
    - 8.2 (3, 11, 21, 31, 41, 55)
    - 8.3 (5, 11, 23, 35, 43, 51, 61)
  - 10 Further applications of trigonometry:
    - 10.3 (1, 9, 13, 21, 29, 41, 53, 57, 63, 65, 73, 75)
    - 10.4 (1, 11, 25, 31, 35, 43, 47, 51, 55, 65)
    - 10.5 (1, 11, 15, 21, 25, 29, 31, 35, 43, 47, 51, 55, 61)
 

## Calculus

Resources:

- Herman, E. & Strang, G. (2020). _**Calculus volume 1**_. OpenStacks. Rice University. Retrieved from [here](https://openstax.org/details/books/calculus-volume-1)

Chapters, sections, and exercises:

- **Functions & Graphs**
  - 1 Functions and graphs:
    - 1.1 Review of functions (exercises: 1, 11, 18, 29, 40, 47, 52)
    - 1.2 Basic classes of functions (63, 71, 89, 94, 110)
    - 1.3 Trigonometric functions (115, 120, 123-128,132, 136, 144, 150, 160, 170)
    - 1.4 Inverse functions (184, 186, 191, 203, 208, 211, 213, 218)
    - 1.5 Exponential and logarithmic functions (231, 233-240, 244, 264-269, 279, 289)
- **Limits and continuity**
  - 2 Limits:
    - 2.1 Calculus preview (1, 3)
    - 2.2 Limit of a function (30, 31, 35, 59-64, 77)
    - 2.3 Limit laws (83-85, 90, 102, 107, 117)
    - 2.4 Continuity (135, 136, 139, 140, 145, 153, 154, 157)
    - 2.5 Precise definition of a limit (177, 188, 192)
- **Derivative of a function (algebraic, trigonometric)**
  - 3 Derivatives:
    - 3.1 Defining the derivative (3, 13, 23, 27, 31, 45)
    - 3.2 The derivative as a function (54–63, 64, 75, 79, 85, 99)
    - 3.3 Differentiation rules (109, 117, 119, 127, 133, 141)
    - 3.4 Derivatives as Rates of change (153, 161, 163)
    - 3.5 Derivatives of trigonometric functions (175, 177, 181, 187, 197, 203)
- **The Chain rule & Implicit differentiation**
  - 3 Derivatives:
    - 3.6 The Chain rule (217, 223, 227, 234, 236, 241, 243, 256)
    - 3.8 Implicit differentiation (301, 307, 311, 316, 323)
- **Derivative of a function (Inverse, Exponential, Logarithmic)**
  - 3 Derivatives:
    - 3.7 Derivatives of Inverse functions (265, 269, 275, 279, 281, 299)
    - 3.9 Derivatives of Exponential and Logarithmic functions (331, 337, 340, 347, 356, 361)
- **Applications, Function's extreme values, & Mean Value Theorem**
  - 4 Applications of derivatives:
    - 4.1 Related rates (1, 5, 7, 17, 29)
    - 4.2 Linear approximations and differentials (50, 54, 57, 77, 87)
    - 4.3 Maxima and Minima (101, 106, 109, 111, 115, 131)
    - 4.4 The Mean Value theorem (157, 161, 163, 172, 176, 190)
    - 4.5 Derivatives and the Shape of a graph (205, 207, 210, 217, 225, 233, 241)
- **Newton's method, Antiderivative, & L'Hopital's rule**
  - 4 :
    - 4.8 (356, 362, 370, 382, 401)
    - 4.9 (407, 420, 428)
    - 4.10 (466, 471, 476, 481, 492, 500, 513)
- **Integrals (Indefinite, Definite) & Fundamental Theorem of Calculus**
  - 4 Applications of derivatives:
    - 4.8 L'Hôpital's rule
    - 4.9 Newton's method
    - 4.10 Antiderivatives
  - 5 Integration:
    - 5.1 Approximating areas (14, 15, 25, 39, 43)
    - 5.2 The Definite integral (61, 77, 81, 93, 99, 111, 115)
    - 5.3 The Fundamental theorem of calculus (149, 155, 171, 177, 185, 199)
    - 5.4 Integration formulas and the Net change theorem (207, 211, 234, 247)
    - 5.5 Substitution (257, 261, 265, 269, 273, 279, 293, 301)
    - 5.6 Integrals involving Exponential and Logarithmic functions (321, 322, 324, 325, 330, 334, 335, 337, 355)


## Discrete mathematics

Main resources:

- Doerr, A., & Levasseur, K. (2024). _**Applied discrete structures**_ (3rd ed., Version 11). Licensed Under a Creative Commons Attribution 4.0. Retrieved from [here](https://discretemath.org/ads/index-ads.html).
- Levin, O. (2021). _**Discrete mathematics: An open introduction**_ (3rd ed.). Licensed Under a Creative Commons Attribution 4.0. Retrieved from [here](https://open.umn.edu/opentextbooks/textbooks/394).

Chapters and sections:

- **Set theory & Basics of counting
  - _**Applied discrete structures**_ (2024)
    4. More on Sets
      4.2. Laws of Set theory
  - _**Discrete mathematics: An open introduction**_ (2021)
    0. Introduction and preliminaries
	  0.3 Sets
	1. Counting
	  1.1. Additive and multiplicative principles
  - Videos:
    - GitMind - EN. (2020, November 11). _**How to draw a Venn diagram?**_ [Video](https://youtube.com/watch?v=MjhJfJJEUgE). YouTube.
	- Mathispower4u. (2022a, June 14). _**De Morgan’s laws with Venn diagrams**_ [Video](https://youtube.com/watch?v=7yf92IgREig). YouTube.
	- Mathispower4u. (2022b, June 14). _**Determine the union, intersection, and difference of two sets given as lists**_ [Video](https://youtube.com/watch?v=hoyAndqazzc). YouTube.
	- Mathispower4u. (2022c, June 23). _**Determine the greatest and least values of the cardinality of an intersection and union**_ [Video](https://youtube.com/watch?v=Z5JDQyPMoHc). YouTube.
	- Mathispower4u. (2022d, June 23). _**Determine the sum of the cardinality of the union and intersection of two sets**_ [Video](https://youtube.com/watch?v=QDqVHgRfdpM). YouTube.
	- Mathispower4u. (2022e, June 11). _**Introduction to sets and set notation**_ [Video](https://youtube.com/watch?v=82vEUReURYw). YouTube.
	- Mathispower4u. (2022f, June 23). _**The cardinality of the union of three sets application: TV shows**_ [Video](https://youtube.com/watch?v=1kFtj2WZAHQ). YouTube.

- **Algebraic structure & Combinatorics**
  - _**Applied discrete structures**_ (2024)
    2. Combinatorics
	  2.2 Permutations
	  2.4 Combinations and the Binomial theorem
	11. Algebraic structures
	  11.1. Operations
	  11.2 Algebraic systems
	  11.3. Some general properties of groups
	  11.4. Greatest common divisors and the integers modulo n
  - _**Discrete mathematics: An open introduction**_ (2021)
    1. Counting
	  1.2. Binomial coefficients
	  1.3. Combinations and Permutations
	  1.4. Combinatorial proofs
	  1.6. Advanced counting using PIE
  - Videos:
    - Learnifyable. (2015a, April 6). (Abstract algebra 1) _**Integers modulo n**_ [Video](https://youtube.com/watch?v=NaFnlpiPNcw). YouTube.
	- Mathispower4u. (2022b, June 24). _**Introduction to bit strings**_ [Video](https://youtube.com/watch?v=RvrXUyhLvEM). YouTube.
	- Mathispower4u. (2022c, June 27). _**Combinations: The number of 4-topping pizzas**_ [Video](https://youtube.com/watch?v=cSoVmzzntag). YouTube.
	- Mathispower4u. (2022g, June 29). _**Counting: Books on a shelf - arrangements and alphabetized**_ [Video](https://youtube.com/watch?v=zQEYk-BL2B8). YouTube.
	- Mathispower4u. (2022d, June 27). _**Evaluating combinations using Pascal's triangle**_ [Video](https://youtube.com/watch?v=aVFEqhaBwcM). YouTube.
	- Mathispower4u. (2022e, June 27). _**Introduction to binomial coefficients**_ [Video](https://youtube.com/watch?v=bqVbOT0Rrcw). YouTube.
	- Mathispower4u. (2022a, July 9). _**Introduction to derangements**_ [Video](https://youtube.com/watch?v=rj4wveoFLD0). YouTube.
	- Neso Academy. (2021a, September 8). _**Modular arithmetic (Part 1)**_ [Video](https://youtube.com/watch?v=M42uDLGRSpI). YouTube.
  - Optional:
	- Learnifyable. (2015b, May 16). _**(Abstract algebra 1) Units modulo n**_ [Video](https://youtube.com/watch?v=LWyH25nzbt0). YouTube. 

- **Introduction to Logic**
  - _**Applied discrete structures**_ (2024)
    3. Logic
	  3.1. Propositions and Logical operators
	  3.2. Thruth tables and Propositions generated by a set
	  3.3. Equivalence and Implication
	  3.4. The Laws of Logic
	  3.5. Mathematical systems and Proofs
	  3.8. Quantifiers
  - _**Discrete mathematics: An open introduction**_ (2021)
    0. Introduction & Preliminaries
	  0.2. Mathematical statements
  - Videos:
    - Mathispower4u. (2022a, July 23). _**Introduction to logically equivalent statements**_ [Video](https://youtube.com/watch?v=OoqINArRp98). YouTube.
	- Mathispower4u. (2022b, July 23). _**Introduction to propositional logic and truth tables**_ [Video](https://youtube.com/watch?v=LUxVpXJlTmk). YouTube.
	- Mathispower4u. (2022c, July 25). _**Deduction rules: Modus Ponens and Modus Tollens**_ [Video](https://youtube.com/watch?v=fdDdkpzANNo). YouTube.
	- Mathispower4u. (2022d, July 25). _**Introduction to predicates and quantifiers**_ [Video](https://youtube.com/watch?v=S2GTUL0cwy4). YouTube.
	- Mathispower4u. (2022e, July 25). _**Introduction to predicate logic**_ [Video](https://youtube.com/watch?v=nFae9CNBNoA). YouTube.
	- Mathispower4u. (2022f, July 25). _**Simplify statements using logically equivalent statements**_ [Video](https://youtube.com/watch?v=MCobYq4kRzU). YouTube. 

- **Mathematical Induction & Relations**
  - _**Applied discrete structures**_ (2024)
    3. Logics
	  3.7. Mathematical induction
	6. Relations
	  6.1. Basic definitions
	  6.2. Graphs of relations on a set
	  6.3. Properties of relations
	  6.4. Matrices of relations
	  6.5. Closure operations on relations
  - _**Discrete mathematics: An open introduction**_ (2021)
    2. Sequences
	  2.5. Induction
  - Videos:
    - Brehm, K. (2020, April 20). _**Discrete math - 9.5.1 equivalence relations**_ [Video](https://youtube.com/watch?v=ZgcTX16borA). YouTube.
	- JF Sherazi. (2018, May 8). _**Partial order relation**_ [Video](https://youtube.com/watch?v=p2fnDkcwiYc). YouTube.
	- Mathispower4u. (2022a, August 18). _**Introduction to congruence modulo n**_ [Video](https://youtube.com/watch?v=LUxVpXJlTmk). YouTube.
	- Mathispower4u. (2022b, July 20). _**Introduction to proof by induction: Prove 1+3+5+…+(2n-1) = n²**_ [Video](https://youtube.com/watch?v=fPX1UBQKcSM). YouTube.
	- Mathispower4u. (2022c, July 21). _**Proof by induction: Prove n² < 2ⁿ for n ≥ 5**_ [Video](https://youtube.com/watch?v=TX6KB7iXyqg). YouTube.

- **Functions & Sequences**
  - _**Applied discrete structures**_ (2024)
    7. Functions
	  7.1. Definition and Notation
	  7.2. Properties of Functions
	  7.3. Function composition
	8. Recursion and Recurrence relations
	  8.2. Sequences
  - _**Discrete mathematics: An open introduction**_ (2021)
    0. Introduction & Preliminaries
	  0.4.
	2. Sequences
	  2.1. Definitions
	  2.2. Arithmetic and Geometric sequences
  - Videos:
    - Mathispower4u. (2022a, June 16). _**Introduction to functions (discrete math)**_ [Video](https://youtube.com/watch?v=WQomLK0vTQ8). YouTube.
	- Mathispower4u. (2022b, June 17). _**Surjective, injective, and bijective functions**_ [Video](https://youtube.com/watch?v=3UpO2zeQKzU). YouTube.
	- Mathispower4u. (2022c, June 18). _**Determine if functions given in two-line notation are surjective, injective, and bijective**_ [Video](https://youtube.com/watch?v=4L6uvT18M2M). YouTube.
	- Mathispower4u. (2022d, June 18). _**Determine if various functions are surjective, injective, and bijective**_ [Video](https://youtube.com/watch?v=i2D8g-i8hXw). YouTube.
	- Mathispower4u. (2022e, June 18). _**List functions given domain and codomain then determine if surjective, injective, and bijective (8)**_ [Video](https://youtube.com/watch?v=QjA0_xYe5DY). YouTube.
	- Mathispower4u. (2022f, June 18). _**List functions given domain and codomain then determine if surjective, injective, and bijective (9)**_ [Video](https://youtube.com/watch?v=YMoMWeR-rDU). YouTube.
	- Mathispower4u. (2022g, July 12). _**Introduction to sequences (discrete math)**_ [Video](https://youtube.com/watch?v=rvjj_SSYiHI). YouTube.
  - Optional:
	- Mathispower4u. (2022c, June 16). _**Describing functions (discrete math)**_ [Video](https://youtube.com/watch?v=eAUBd0e2RRA). YouTube.
	- Mathispower4u. (2022a, July 12). _**Determine a closed formula for a given sequence (1)**_ [Video](https://youtube.com/watch?v=4YxqncJCack). YouTube.
	- Mathispower4u. (2022j, June 18). _**Two-line notation for a function: Inputs and outputs**_ [Video](https://youtube.com/watch?v=F43tjoFx3OU). YouTube.

- **Recursion & Solutions of Recurrence relations**
  - _**Applied discrete structures**_ (2024)
    8. Recursion & Recurrence relations
	  8.1. The many faces of Recursion
	  8.3. Recurrence relations
  - _**Discrete mathematics: An open introduction**_ (2021)
    2. Sequences
	  2.4. Solving recurrence relations
  - Videos:
    - Brehm, K. (2020, March 4). _**Discrete math - 2.4.2. recurrence relations**_ [Video](https://youtube.com/watch?v=QaAQ-bKCUME). YouTube.
	- Malozzi, J. (2021a, February 21). _**Solving a recurrence (Iterative approach)**_ [Video](https://youtube.com/watch?v=q2sWv4BfG68). YouTube.
	- Malozzi, J. (2021, February 22). _**Linear homogeneous recurrences with constant coefficients**_ [Video](https://youtube.com/watch?v=z2dSnkO6Ufo). YouTube.
	- Malozzi, J. (2021b, February 21). _**Solving a linear homogeneous recurrence with constant coefficients (Fibonacci sequence)**_ [Video](https://youtube.com/watch?v=Vft84e5p5SI). YouTube.
	- Mathispower4u. (2022, July 18). _**Sequences: Introduction to solving recurrence relations**_ [Video](https://youtube.com/watch?v=DTERgFa9jJc). YouTube.
	- Mathispower4u. (2022a, July 19). _**Solve a recurrence relation using the characteristic root technique (repeated root)**_ [Video](https://youtube.com/watch?v=2tEOFuIEyzc). YouTube.
	- Mathispower4u. (2022b, July 19). _**Solve a recurrence relation using the characteristic root technique (2 distinct roots)**_ [Video](https://youtube.com/watch?v=I3j1LuDq2yE). YouTube.

- **Graph theory I**
  - _**Applied discrete structures**_ (2024)
    9. Graph theory
	  9.1. Graphs - General introduction
	  9.3. Connectivity
	  9.4. Traversals: Eulerian and Hamiltonian graphs
	  9.5. Graph optimization
  - _**Discrete mathematics: An open introduction**_ (2021)
    4. Graph theory
	  4.1. Definitions
	  4.5. Euler path and circuits
	  4.7. Chapter summary
  - Videos:
    - Computer Science Lesson. (2016, May 16). _**Graph data structure 4: Dijkstra's shortest path algorithm**_ [Video](https://youtube.com/watch?v=pVfj6mxhdMw). YouTube.
	- CSU Mathematics. (2021, August 6). _**Graph optimization**_ [Video](https://youtube.com/watch?v=ld1tR8Oom5U). YouTube.
	- Mathispower4u. (2022a, August 2). _**Introduction to graph theory**_ [Video](https://youtube.com/watch?v=RWDKNOoU_KI). YouTube.
	- Mathispower4u. (2022b, August 9). _**Introduction to Euler paths and Euler circuits**_ [Video](https://youtube.com/watch?v=J6O9kis_pTw). YouTube.
	- Mathispower4u. (2022c, August 9). _**Introduction to Hamilton paths and Hamilton circuits**_ [Video](https://youtube.com/watch?v=UOE22pyKD5c). YouTube.
  - Optional:
	- Humberstone, B. (2021, August 11). _**Dijkstra's algorithm (finding shortest path)**_ [Video](https://youtube.com/watch?v=g5OV12u3uHY). YouTube.
	- Just Entertainment. (2017, July 19). _**Dijkstra's algorithm shortest path selection tutorial**_ [Video](https://youtube.com/watch?v=HVYi_ttE1eQ). YouTube.
	- Mathispower4u. (2022d, August 2). _**Isomorphic graphs**_ [Video](https://youtube.com/watch?v=9UbC7p18PDw). YouTube.
	- Mathispower4u. (2022e, August 2). _**The handshake lemma**_ [Video](https://youtube.com/watch?v=i5QYTTioZu4). YouTube.
	- Olsen, J. (2015, November 4). _**Euler and Hamiltonian paths and circuits**_ [Video](https://youtube.com/watch?v=AwsMTEl79wI). YouTube.

- **Graph theory II**
  - _**Applied discrete structures**_ (2024)
    9. Graph theory
	  9.6. Planarity and Colorings
	10. Trees
	  10.1. What is a tree?
	  10.2. Spanning trees
	  10.3. Rooted trees
  - _**Discrete mathematics: An open introduction**_ (2021)
    4. Graph theory
	  4.1. Definitions
	  4.2. Trees
	  4.3. Planar graphs
	  4.4. Coloring
	  4.7. Chapter summary
  - Videos:
    - Mathispower4u. (2022a, August 4). _**Introduction to rooted trees**_ [Video](https://youtube.com/watch?v=DdNFA74H0ow). YouTube.
	- Mathispower4u. (2022b, August 4). _**Introduction to spanning trees**_ [Video](https://youtube.com/watch?v=fO-R1vwgsmw). YouTube.
	- Mathispower4u. (2022c, August 4). _**Introduction to trees and properties of trees**_ [Video](https://youtube.com/watch?v=43d1tdR6-Vw). YouTube.
	- Mathispower4u. (2022d, August 6). _**Non-planar graphs: Proving K₅ and K₃,₃ are non-planar**_ [Video](https://youtube.com/watch?v=7Zv-PK2uoSo). YouTube.
	- Mathispower4u. (2022e, August 8). _**Introduction to vertex coloring and the chromatic number of a graph**_ [Video](https://youtube.com/watch?v=zNipKCWWA4c). YouTube.
  - Optional:
    - Mathispower4u. (2022f, August 19). _**Introduction to planar graphs and Euler's formula**_ [Video](https://youtube.com/watch?v=1Fc-ustB_Rg). YouTube.
	- Inside Code. (2020, July 29). _**How to solve (almost) any binary tree coding problem**_ [Video](https://youtube.com/watch?v=s2Yyk3qdy3o). YouTube.
	- Mathispower4u. (2022h, August 4). _**Given a rooted tree, determine relationships**_ [Video](https://youtube.com/watch?v=1Fc-ustB_Rg). YouTube.
	- Ms. Hearn. (2020, February 17). _**Recognizing and finding spanning trees in graph theory**_ [Video](https://youtube.com/watch?v=FfgXmie2NxY). YouTube.

## Introduction to statistics

Resources:

- Illowsky, B., Dean, S., Birmajer, D., Blount, B., Boyd, S., Einsohn, M., Helmreich, J., Kenyon, L., Lee, S., & Taub, J. (2022). _**Introductory statistics**_. Openstax. Retrieved from [here](https://openstax.org/details/books/introductory-statistics).

Chapters and sections:

- **Sampling data**
  - 1 Sampling and Data:
    - 1.1 Definitions of Statistics, Probability, and Key terms
    - 1.2 Data, Sampling, and Variation in Data and Sampling
    - 1.3 Frequency, Frequency tables, and Levels of measurement
- **Descriptive analysis**
  - 2 Descriptive statistics:
    - 2.1 Stem-and-leaf graphs (Stemplots), Line graphs, and Bar graphs
    - 2.2 Histograms, Frequency polygons, and Time series graphs
    - 2.3 Measures of the location of the data
    - 2.4 Box plots
    - 2.5 Measures of the center of the data
    - 2.6 Skewness and the Mean, Median, and Mode
    - 2.7 Measures of the Spread of the data
    - 2.8 Descriptive statistics
- **Probability**
  - 3 Probability topics:
    - 3.1 Terminology
    - 3.2 Independent and mutually exclusive events
    - 3.3 Two basic rules of probability
    - 3.4 Contingency tables
    - 3.5 Tree and Venn diagrams
    - 3.6 Probability topics
- **Discrete random variables**
  - 4 Discrete random variables:
    - 4.1 Probability distribution function (PDF) for a Discrete random variable
    - 4.2 Mean or Expected value and Standard deviation
    - 4.3 Binomial distribution
- **Geometric, Hyper-geometric, & Poisson distribution**
  - 4 Discrete random variables:
    - 4.5 Hypergeometric distribution
    - 4.6 Poisson distribution
- **Continuous random variables**
  - 5 Continuous random variables:
    - 5.1 Continuous probability functions
    - 5.2 Uniform distribution
    - 5.3 Exponential distribution
- **The Normal distribution**
  - 6 Normal distribution:
    - 6.1 Standard normal distribution
    - 6.2 Using the Normal distribution
- **The Central limit theorem**
  - 7 The Central limit theorem:
    - 7.1 The Central limit theorem for sample means (averages)
    - 7.2 The Central limit theorem for sums
    - 7.3 Using the Central limit theorem
    - 7.4 Central limit theorem (pocket change)
    - 7.5 Central limit theorem (cookie recipes)


## Statistical inference

Resources:

- David Diez, Mine Cetinkaya-Rundel, Christopher D. Barr (2019) _**OpenIntro Statistics (4th ed.)**_. OpenIntro. Retrieved from [here](https://openintro.org/book/os/).
- Mark A. Goss-Sampson (2022) _**Statistical analysis in JASP. A guide for students (5th ed.)**_. JASP. Retrieved from [here](https://jasp-stats.org/wp-content/uploads/2022/04/Statistical-Analysis-in-JASP-A-Students-Guide-v16.pdf).
- [JASP software](https://jasp-stats.org/)

Chapters and sections:

- **Foundations for inference & Introduction to JASP (Jeffrey's Amazing Statistics Program)**

  - _**OpenIntro Statistics (4th ed.)**_
    5. Foundations for inference
      5.1. Point estimates and sampling variability
      5.2. Confidence intervals for a proportion

  - _**Statistical analysis in JASP. A guide for students**_
    - Read pages 2-31

  - OpenIntroOrg. (2019a, September 02). Foundations for inference: Point estimates [Video]. YouTube. Retrieved from [here](https://youtu.be/oLW_uzkPZGA).
  - OpenIntroOrg. (2019b, September 6). Intro to confidence intervals via proportions [Video]. YouTube. Retrieved from [here](https://youtu.be/A6_W8qY8zJo).
  - JASP Statistics. (2022, October 05). Introduction to JASP [Video]. YouTube. Retrieved from [here](https://youtu.be/APRaBFC2lEQ).

- **Hypothesis testing and Inference for categorical data**

  - _**OpenIntro Statistics (4th ed.)**_
    5. Foundations for inference
      5.3. Hypothesis testing for a proportion
    6. Inference for categorical data
      6.1. Inference for a single proportion
      6.2. Difference of two proportions

- The Organic Chemistry Tutor. (2019a, October 28). Hypothesis testing - solving problems with proportions [Video]. YouTube. Retrieved from [here](https://youtu.be/76VruarGn2Q).
- The Organic Chemistry Tutor. (2019b, November 15). Hypothesis testing with two proportions [Video]. YouTube. Retrieved from [here](https://youtu.be/pCbNUnZ98oE).
- Introduction to Statistics at SLCC. (2021, June 18). Chapter 3.5 - lesson 1/3 - inference for a single proportion [Video]. YouTube. Retrieved from [here](https://youtu.be/VB8kttv9hoY).

- **Testing for goodness of fit and Independence**

  - _**OpenIntro Statistics (4th ed.)**_
    6. Inference for categorical data
      6.3. Testing for goodness of fit using chi-square
      6.4. Testing for independence in two-way tables

- Çetinkaya-Rundel, M. (2018a, February 20). 6 3 Testing for goodness of fit using chi square [Video]. YouTube. Retrieved from [here](https://youtu.be/Uk36WGxujkc).
- Çetinkaya-Rundel, M. (2018b, February 20). 6 4 Homogeneity and independence in two-way tables [Video]. YouTube. Retrieved from [here](https://youtu.be/yjrsfNdja0U).

- **Inference for numerical data**

  - _**OpenIntro Statistics (4th ed.)**_
    7. Inference for numerical data
      7.1. One sample means with t-distribution
      7.2. Paired data
      7.3. Difference of two means

- Çetinkaya-Rundel, M. (2018a, February 20). 5 1A t distribution [Video]. YouTube. Retrieved from [here](https://youtu.be/uVEj2uBJfq0).
- Çetinkaya-Rundel, M. (2018b, February 20). 5 1B Inference for a mean [Video]. YouTube. Retrieved from [here](https://youtu.be/RYVIGj1l4xs).
- Çetinkaya-Rundel, M. (2018c, February 20). 5 2 Inference for paired data [Video]. YouTube. Retrieved from [here](https://youtu.be/K0QZ9_4w0HU).
- Çetinkaya-Rundel, M. (2018d, February 20). 5 3 Difference of two independent means [Video]. YouTube. Retrieved from [here](https://youtu.be/emZ24asR2F4).

- **Power calculations and ANOVA**

  - _**OpenIntro Statistics (4th ed.)**_
    7. Inference for numerical data
      7.4. Power calculations for a difference of means
      7.5. Comparing many means with ANOVA

  - _**Statistical analysis in JASP. A guide for students**_
    - Read Descriptive statistics (pp. 14-27)
    - Read ANOVA (pp. 91-98)

- Çetinkaya-Rundel, M. (2018a, February 20). 5 4 Power calculations for difference of two means [Video]. YouTube. Retrieved from [here](https://youtu.be/vnjjhQDedvs).
- Çetinkaya-Rundel, M. (2018b, February 20). 5 5A ANOVA introduction [Video]. YouTube. Retrieved from [here](https://youtu.be/W36DMVJ4Ibo).
- Çetinkaya-Rundel, M. (2018c, February 20). 5 5B Conditions for ANOVA [Video]. YouTube. Retrieved from [here](https://youtu.be/HGFiWMA5OC8).
- headlessprofessor. (2016, November 12). JASP ANOVA [Video]. YouTube. Retrieved from [here](https://youtu.be/nlAhWQmG5Iw).

- **Introduction to linear regression**

  - _**OpenIntro Statistics (4th ed.)**_
    8. Introduction to linear regression
      8.1. Fitting a line, residuals, and correlation
      8.2. Least squares regression
      8.3. Types of outliers in linear regression

  - _**Statistical analysis in JASP. A guide for students**_
    - Read Correlation analysis, Running correlation (pp. 67-70)
    - Read Regression (pp. 73-78)

- OpenIntroOrg. (2014a, January 26). Line fitting, residuals, and correlation [Video]. YouTube. Retrieved from [here](https://youtu.be/mPvtZhdPBhQ).
- OpenIntroOrg. (2014b, January 27). Fitting a line with least squares regression [Video]. YouTube. Retrieved from [here](https://youtu.be/z8DmwG2G4Qc).
- OpenIntroOrg. (2014c, February 11). Types of outliers in linear regression [Video]. YouTube. Retrieved from [here](https://youtu.be/jZEKAlo1E54).
- Research By Design. (2020, June 5). How to do simple linear regression in JASP (14-7) [Video]. YouTube. Retrieved from [here](https://youtu.be/vKGphOrzze8).

- **Linear regression inference and Introduction to multiple regression**

  - _**OpenIntro Statistics (4th ed.)**_
    8. Introduction to linear regression
      8.4. Inference for linear regression
    9. Multiple and logistic regression
      9.1. Introduction to multiple regression
      9.2. Model selection

  - _**Statistical analysis in JASP. A guide for students**_
    - Read Multiple regression (pp. 79-85)

- OpenIntroOrg. (2014, January 27). Inference for linear regression [Video]. YouTube. Retrieved from [here](https://youtu.be/depiT-hTaGA).
- OpenIntroOrg. (2013, November 14). Introduction to multiple regression [Video]. YouTube. Retrieved from [here](https://youtu.be/sQpAuyfEYZg).
- OpenIntroOrg. (2013, November 17). Model selection in multiple regression [Video]. YouTube. Retrieved from [here](https://youtu.be/VB1qSwoF-l0).
- Social and Behavioral Sciences at Bethel Univ. (2019, August 20). Multiple regression in JASP [Video]. YouTube. Retrieved from [here](https://youtu.be/7ZQG9DzxLZ8).

- **Logistic regression**

  - _**OpenIntro Statistics (4th ed.)**_
    9. Multiple and logistic regression
      9.3. Checking model conditions using graphs
      9.5. Introduction to logistic regression

  - _**Statistical analysis in JASP. A guide for students**_
    - Read Logistic regression (pp. 86-90)

- OpenIntroOrg. (2013a, November 24). Checking multiple regression diagnostics using graphs [Video]. YouTube. Retrieved from [here](https://youtu.be/3KSUeYMKt5A).
- OpenIntroOrg. (2013b, November 30). Basic ideas of logistic regression [Video]. YouTube. Retrieved from [here](https://youtu.be/uYC2eLVSpI8).
- JASP Statistics. (2018, February 11). How to perform a logistic regression analysis in JASP [Video]. YouTube. Retrieved from [here](https://youtu.be/bUgpJeeReBY).
