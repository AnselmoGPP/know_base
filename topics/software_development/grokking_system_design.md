...
- **Reliability** (different to Fault tolerance)
- **Availability** (different to Reliability)
- **Efficiency**
- **Manageability** (or Serviceability)
  
### Load balancing

- **Load balancer** (LB): It distributes requests across a cluster of servers (improving responsiveness and availability), and keeps track of the resources' status.
- **Health checks**: Regular attempts to connect to servers to ensure that they are listening.
- **Redundant LBs**: 
- **Load balancing algorithms**
  - **Least connection method**
  - **Least response time method**: To the server with lowest average response time. Useful for web services and APIs, and dynamic environments.
...
- Cache invalidation methods
  ...
  - Stale-while-revalidate (SWI)
- Cache read strategies
  - Read through cache
  - Read aside cache (or cache-aside, or lazy loading)
...
- Database types
  ...
  - NoSQL
- SQL databases (Relational databases)
  - Relational data model (relations, entity, foreign-keys, structured and schema-based)
...
- Comparison
  ...
  - Use cases
- What to choose?
  - SQL (structure and relational data, consistency, complex querying, transactions, long-term maintenance and reporting)
  - NoSQL (big scale/throughput, changing data models, many small ops, distributed data, specific data)
...
- CAP theorem: A distributed system cannot provide simultaneously C, A, and P:
  - Consistency (C)
  - Availability (A)
  - Partition tolerance (P)
- New CAP theorem: In the presence of a network partition, a distributed system must choose either C or A.

### PACELC theorem

- ACID (Atomicity, Consistency, Isolation, Durability): Consistency
- BASE (Basically Available, Soft-state, Eventually consistent): Availability
- PACELC theorem (if P, AC; else LC): In a distributed system that replicates data:
  - if there's P (partition (P), it can tradeoff between A and C.
  - else, in the absence of partitions, it can tradeoff between latency (L) and C.
...
- Data replication
- Consistent hashing
  - MD5 (MD5 Message-Digest 5 Algorithm)
- Virtual nodes
  - Basic Consistent hashing
...
- Operations
  - Add an item
  - Query item (check membership)
- No false negative guaranteed
- Redundancy (multiple hash function) make false negatives impossible
- How Bloom filters work
- Usage process
  - Initialize
  - Adding an item
...
- Removing items
- Variants (Counting Bloom filters, …)
- False positives
